...
router.post('/login', function(req, res, next) {
    User.findOne({email: req.body.email}, function(err, user) {
        ...
        var claimToken = {_id: user._id, email: user.email, role: user.role};
...
router.patch('/user-profile', function(req, res, next){
    ...
    User.findById(decoded.user._id, function(err, user){
        ...
        User.findByIdAndUpdate(decoded.user._id, updateUser, function(err, result) {
            ...
            res.json({
                title: 'Success!',
                error: {message:'User profile has been updated'}    //this part has been adjusted just to be able to use "errorHandler" service
                //obj: result
            });
        });
    })
